...
/// 应用维度（App）：供应商归属的 AI CLI 工具。临时最小定义，仅供本批次
/// （#33 codex 写盘）在 #32 数据模型迁移合入前编译与自验——#32 会把它落成
/// 正式实体（provider 表 app 列、按 (app, id) 合并、per-app 激活状态等）。
...
/// A provider (供应商): `settingsConfig` is the app's live-file snapshot —
/// Claude 是 `settings.json` 快照（raw JSON text），Codex 是 `{"auth",
/// "config"}` 快照（auth = auth.json 内容、config = config.toml TOML 文本）；
/// `meta` carries app-side info the live file never sees. `sortIndex` is the
/// user-ordered display rank.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize, Type)]
#[serde(rename_all = "camelCase")]
pub struct Provider {
    /// 归属应用。TEMP-APP-SHIM: #32 合并后移除，以 #32 实现为准——#32 落成
    /// provider 表 app 列；`#[serde(default)]` 让无 app 字段的旧同步文件 /
    /// 导出文档读为 claude（= #32 的存量迁移语义）。
    #[serde(default)]
    pub app: App,
    pub id: String,
    pub name: String,
