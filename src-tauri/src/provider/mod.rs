...
//! 写」语义）、`keys`（密钥位置清单 + strip / restore 成对纯函数：push /
//! 导出剥、pull 回填共用，见本目录 `keys.rs`）、`snippet`（通用配置片段：
//! 手写片段 + 启用开关，写盘时合并进受控字段，存本机 config.json 不同步）、
//! `export_import`（全部供应商导出 / 导入一份 JSON 文档，手动迁移，不走 git
//! 同步），以及 `model_fetch`（拉取供应商的模型列表：OpenAI 兼容
//! `GET /v1/models`，候选 URL 构造是纯函数、失败错误串带分桶标签——见本
//! 目录 `model_fetch.rs`）。
