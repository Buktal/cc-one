...
/// 目录根走 [`App::app_config_dir`]（per-app 家目录映射的单一声明处）。
pub fn claude_settings_path() -> AppResult<PathBuf> {
    Ok(App::Claude.app_config_dir()?.join("settings.json"))
}
...
// TOML 载体共用机制（codex / grok：解析 / 补缺失 / 片段校验与合并骨架 /
// `config` 字段提取）归同目录的 [`live_toml`]；本模块留 JSON 载体与共享的
// 文件事务原语。re-export 保持 `crate::provider::live::` 的既有引用面零变化。
pub(crate) use super::live_toml::{
    config_toml_field, fill_missing_table, merge_toml_snippet, parse_toml_or_empty,
