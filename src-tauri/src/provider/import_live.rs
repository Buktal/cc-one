...
// 任意选择：
// - claude / gemini：片段在 settings_config 层合并（claude 认 env 子键 + 受控
//   非 env 顶层开关，`snippet::merge_snippet_into_settings` 只收
//   `CONTROLLED_FIELDS`；gemini 只认 env 子键）→ 名册 = env 子键 + 顶层开关；
...
/// 不了的键）。settings.json 的键不在名册：提取只取 env，非受控顶层键进片段
/// 零效果。
